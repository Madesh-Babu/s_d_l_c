...
class CategoryService(
    ICategoryUpdater, ICategoryReader, ICategoryCreator, ICategoryDeleter
):
    ...
    def apply_discount(product, discount_percentage, tax_percentage=0):
        ...
        try:
            discount_request = DiscountRequest(
                id=product.id, discount=discount_percentage, tax=tax_percentage
            )
            ...
            final_price = (
                TaxDecorator(discounted, tax_percentage).get_price()
                if tax_percentage > 0
                else discounted.get_price()
            )

...
def create_app(config_name=None):
    ...
    try:
        ...
        logger.info(
            "Application configured with new system",
        ...
        app.config["SQLALCHEMY_DATABASE_URI"] = (
            f"postgresql://{os.getenv('PG_USER', 'postgres')}:{os.getenv('PG_PASSWORD', 'postgresql')}@{os.getenv('PG_HOST', '127.0.0.1')}:{os.getenv('PG_PORT', '5432')}/{os.getenv('PG_DB', 'Inventory_Management_API')}"
        )

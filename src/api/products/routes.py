...
def add_product():
    ...
    try:
        ...
        try:
            ...
            product = product_service.create_product(data)
            return (
                jsonify({"message": "Product added", "product": product.to_dict()}),
                201,
            )
        except ValidationError as e:
            return jsonify({"error": str(e)}), 400
...
def update_product():
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Product updated", "product": updated_product.to_dict()}
            ),
            200,
        )
    except ValidationError as e:
...
def discount_product():
    ...
    try:
        ...
        updated = DiscountedProductService.apply_discount(
            product, discount_request.discount, discount_request.tax
        )
        return (
            jsonify(
                {

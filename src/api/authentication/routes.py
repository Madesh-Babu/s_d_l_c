    ...
    try:
        ...
        if not data:
            raise ValidationErrorException(
                "No data provided",
                details={"required_fields": ["username", "email", "password", "role"]},
            )
        ...
        try:
            ...
            password_strength = PasswordValidator.get_password_strength(
                user_data.password
            )
            ...
            raise ValidationErrorException(
                str(e), details={"validation_type": "email_password"}
            )
        ...
        logger.error(
            "Registration failed with unexpected error", error=str(e), exc_info=True
        )
        return exception_handler.handle_exception(e)
...
def login():
    ...
    try:
        ...
        try:
            ...
            raise DatabaseError(
                "Failed to generate access token", details={"user_id": user.id}
            )
        ...
        log_user_action(
            "user_login", str(user.id), username=user.username, role=user.role
        )
...
def change_password():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "New password validation passed",
                user_id=current_user_id,
                password_strength=password_strength["strength"],
            )
            ...
            logger.warning(
                "Password change failed - validation errors",
                user_id=current_user_id,
                errors=str(e),
            )
                ...
                jsonify(
                    {
                        ...
                        "password_strength": (
                            password_strength
                            if "password_strength" in locals()
                            else None
                        ),
                    }
                ),
            ...
            return (
                jsonify({"error": "Password validation failed", "details": str(e)}),
                400,
            )
    ...
    return jsonify(
        {
            ...
            "both_conditions": settings.feature_toggles.BYPASS_AUTH
...
def validate_password():
    ...
    validation_result = validator.validate_user_registration(
        email or "test@example.com", password, username
    )

...
class FeatureToggles(BaseSettings):
    ...
    BYPASS_AUTH: bool = Field(
        False, description="Bypass authentication in local development"
    )
    ENABLE_JWT: bool = Field(True, description="Enable JWT authentication")
    ENABLE_ROLE_BASED_ACCESS: bool = Field(
        True, description="Enable role-based access control"
    )
    ...
    ENABLE_REQUEST_LOGGING: bool = Field(
        True, description="Enable detailed request logging"
    )
    ...
    def validate_bypass_auth(cls, v, info):
        ...
        if v and env not in ["development", "local", "dev"]:
            raise ValueError(
                "Authentication bypass can only be enabled in development environment"
            )
        return v
...
class EnvironmentConfig(BaseSettings):
    ...
    JWT_ACCESS_TOKEN_EXPIRES: int = Field(
        3600, description="JWT access token expiration in seconds"
    )
...
class DevelopmentConfig(EnvironmentConfig):
    ...
    feature_toggles: FeatureToggles = Field(
        default_factory=lambda: FeatureToggles(
...
class ProductionConfig(EnvironmentConfig):
    ...
    FLASK_DEBUG: bool = Field(
        False, description="Flask debug mode disabled in production"
    )

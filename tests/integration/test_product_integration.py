...
class TestProductIntegration:
    ...
    def setup_test_data(self, app, client):
        ...
        with app.app_context():
            ...
            test_category = Category(
                name="test_category", description="Test category for integration tests"
            )
    ...
    def get_auth_headers(self, client):
        ...
        login_data = {"username": "test_product_user", "password": "TestPassword123!"}
    ...
    def test_product_filtering_and_pagination(self, client):
        ...
        response = client.get(
            "/products/?min_price=20&max_price=40", headers=auth_headers
        )
        ...
        response = client.get(
            f"/products/?category_id={self.category_id}", headers=auth_headers
        )
    ...
    def test_product_category_relationships(self, client):
        ...
        our_product = next(
            (p for p in result["products"] if p["id"] == product_id), None
        )
    ...
    def test_product_concurrent_operations(self, client):
        ...
        update_data_1 = {"price": 39.99, "stock_quantity": 80}

        update_data_2 = {"price": 49.99, "stock_quantity": 60}
    ...
    def test_product_data_integrity(self, client):
        ...
        list_product = next(
            (p for p in response_list.get_json()["products"] if p["id"] == product_id),
            None,
        )

...
def sample_user(app):
    ...
    with app.app_context():
        user = User(username="sample_user", email="sample@test.com", role="staff")
...
def sample_category(app):
    ...
    with app.app_context():
        category = Category(
            name="Sample Category", description="Sample category for testing"
        )
...
def manager_user(app):
    ...
    with app.app_context():
        manager = User(
            username="manager_user", email="manager@test.com", role="manager"
        )
...
def staff_user(app):
    ...
    with app.app_context():
        staff = User(username="staff_user", email="staff@test.com", role="staff")
...
def auth_headers(client, sample_user):
    ...
    login_data = {"username": "sample_user", "password": "SamplePassword123!"}
...
def admin_headers(client, admin_user):
    ...
    login_data = {"username": "admin_user", "password": "AdminPassword123!"}
...
def manager_headers(client, manager_user):
    ...
    login_data = {"username": "manager_user", "password": "ManagerPassword123!"}
...
def staff_headers(client, staff_user):
    ...
    login_data = {"username": "staff_user", "password": "StaffPassword123!"}
...
def hierarchical_categories(app):
    ...
    with app.app_context():
        # Parent categories
        electronics = Category(name="Electronics", description="All electronic devices")
        computers = Category(
            name="Computers",
...
class IntegrationTestHelpers:
    ...
    def create_product_with_auth(
        client, headers, name, description, price, stock, category_id
    ):
    ...
    @staticmethod
    def assert_response_status_and_error(
        response, expected_status, expected_error_substring=None
    ):
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "product: marks tests as product tests")
    config.addinivalue_line("markers", "category: marks tests as category tests")

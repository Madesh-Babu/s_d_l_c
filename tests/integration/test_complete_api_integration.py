...
class TestCompleteAPIIntegration:
    ...
    def test_complete_inventory_management_workflow(self, client):
        ...
        # Step 2: Admin login
        login_data = {"username": "test_admin", "password": "AdminPassword123!"}
        ...
        # Step 6: Staff login
        staff_login = {"username": "test_staff", "password": "StaffPassword123!"}

        response = client.post(
            "/auth/login", data=json.dumps(staff_login), content_type="application/json"
        )
        ...
        response = client.get(
            "/products/?min_price=30&max_price=100", headers=staff_headers
        )
        assert response.status_code == 200
        ...
        response = client.get(
            f'/products/?category_id={computers_category["id"]}', headers=staff_headers
        )
        ...
        update_data = {"stock_quantity": 45}  # Reduced from 50

        response = client.put(
            f'/products/{laptop_product["id"]}',
        ...
        # Verify update
        response = client.get(
            f'/products/{laptop_product["id"]}', headers=staff_headers
        )
    ...
    def test_multi_user_role_based_workflow(self, client):
        ...
        for user in roles_users:
            ...
            test_category = next(
                (c for c in categories if c["name"] == "Multi-user Test Category"), None
            )
        ...
        for user in roles_users:
            ...
            test_product = next(
                (p for p in products if p["name"] == "Multi-user Test Product"), None
            )
            assert test_product is not None

        # Admin updates product
        update_data = {"price": 89.99, "stock_quantity": 80}
    ...
    def test_error_recovery_and_data_consistency(self, client):
        ...
        invalid_update = {"price": -5.99}  # Invalid negative price
    ...
    def test_concurrent_operations_simulation(self, client):
        ...
        all_products = response.get_json()["products"]
        concurrent_products = [
            p for p in all_products if p["name"].startswith("Concurrent Product")
        ]
        assert len(concurrent_products) == 5

        # Simulate concurrent updates
        for i, product in enumerate(products):
            update_data = {"stock_quantity": 100 + i * 5}  # Update stock
